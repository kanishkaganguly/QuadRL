...
def is_valid_state(pos_start,pos_current):
    valid= True
...
def main():
    ...
    while (vrep.simxGetConnectionId(clientID) != -1):
        ...
        for iter in range(refresh_freq):
            ...
            valid=is_valid_state(pos_start,pos_new)
            if valid:
                next_state = np.concatenate([pos_new - pos_old,euler_new-euler_old])
            else:
                next_state=None

            next_state = torch.from_numpy(np.asarray(next_state, dtype=np.float32)).view(-1, n_input) if next_state is not None else None

            reward=np.float32(-np.linalg.norm(pos_new -pos_old))

            memory.push(state, torch.from_numpy(np.asarray([[action]])), next_state, torch.from_numpy(np.asarray([[reward]])))
